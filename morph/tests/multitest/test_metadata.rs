...
fn querying_metadata_should_work() {
    // Prepare the chain.
    let mut app = AppBuilder::new().with_api(MockApiBech32::new("wasm")).build(no_init);
    ...
    let contract_addr = app.instantiate_contract(code_id, owner_addr.clone(), &Empty {}, &[], "my-intro-1", None).unwrap();
    ...
    //
    let code_info_response = app.wrap().query_wasm_code_info(contract_info_response.code_id).unwrap();
    assert_eq!(1, code_info_response.code_id);
    assert_eq!(creator_addr.as_str(), code_info_response.creator.as_str());
    assert_eq!(CHECKSUM, code_info_response.checksum.to_string().to_uppercase());
}
